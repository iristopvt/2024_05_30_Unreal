...
void Player::DFS(Vector2 start)
{
	...
	_visited[start._y][start._x] = true;
...
void Player::Astart(Vector2 start, Vector2 end)
{
	...
	startV.g = 0;
	pq.push(startV);
	startV.h = start.MangattanDistance(_maze->GetEndPos()) * 10.0f; // 44 ?
	...
	while (true)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			...
			int h = there.MangattanDistance(_maze->GetEndPos()) * 10.0f;
			int f = newCost + h;
