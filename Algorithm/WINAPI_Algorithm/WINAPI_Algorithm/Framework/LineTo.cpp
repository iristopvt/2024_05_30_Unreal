...
HResult Line::IsCollision(shared_ptr<Line> other)
{
	...
	// 
	double a1 = (double)(other->_start._x - _start._x) * (_end._y - _start._y) - (other->_start._y - _start._y) * (_end._x - _start._x);
	double a2 = (double)(other->_end._x - _start._x) * (_end._y - _start._y) - (other->_end._y - _start._y) * (_end._y - _start._x);    
	double a3 = (double)(_start._x - other->_start._x) * (other->_end._y - other->_start._y) - (_start._y - other->_start._y) * (other->_end._x -other->_start._x);
	double a4 = (double)(_end._x - other->_start._x) * (other->_end	._y - other->_start._y) - (_end._y - other->_start._y) * (other->_end._x - other->_start._x);
